...
export default function BudgetFicheDialog({ open, onOpenChange, line, year, scenario = "ca", locks = {}, isAdmin = true, onSaved }) {
  ...
  const save = async () => {
    if (locked) { toast.error(t("Budget verrouillé — seul un administrateur peut modifier.")); return; }
    try {
      await api.saveBudgetOverride(line.employee_id, override, { year, scenario: scn });
      toast.success(`${t(SCEN_LABEL[scn])} ${t("enregistré")}`); await refetch(); onSaved(scn);
    } catch (e) { toast.error(e.response?.data?.detail || t("Enregistrement impossible")); }
  };
  const reset = async () => {
    if (locked) { toast.error(t("Budget verrouillé — seul un administrateur peut modifier.")); return; }
    try { await api.saveBudgetOverride(line.employee_id, {}, { year, scenario: scn }); toast.success(t("Ligne réinitialisée")); await refetch(); onSaved(); }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[92vh] max-w-4xl overflow-y-auto" data-testid="budget-fiche-dialog">
        <DialogHeader>
          <DialogTitle>{t(SCEN_LABEL[scn])} {year} — {line.name}</DialogTitle>
        ...
        <div className="flex flex-wrap items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3" data-testid="fiche-scenario-bar">
          <span className="text-[11px] font-700 uppercase tracking-widest text-slate-500">{t("Saisie")}</span>
          ...
          <span className="text-[11px]" style={{ color: revueMode ? "#8B5CF6" : "#64748B" }}>
            {revueMode
              ? t("Revue Budgétaire : ajustez primes, augmentation et vacances — le Budget CA n'est pas modifié.")
              : t("Budget CA : bâti à partir du salaire actuel + augmentations/primes.")}
          </span>
          {locked && <span className="flex items-center gap-1 rounded-full bg-red-50 px-2.5 py-1 text-[11px] font-700 text-red-600" data-testid="fiche-lock-badge">🔒 {t("Verrouillé — lecture seule")}</span>}
        </div>

        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          <div className="space-y-4">
            <div className="rounded-xl border border-slate-200" data-testid="fiche-emploi-section">
              <div className="border-b border-slate-200 bg-slate-50 px-3 py-2"><h3 className="text-xs font-700 uppercase tracking-widest">{t("Emploi & imputation")}</h3></div>
              <div className="grid grid-cols-2 gap-3 p-3">
                <div className={isPartTime ? "" : "col-span-2"}><Label className="text-[11px] uppercase text-slate-500">{t("Type d'emploi")}</Label>
                  <Select value={f.employment_type} onValueChange={(v) => set("employment_type", v)}>
                    ...
                    <SelectContent>{EMP_TYPES.map((x) => <SelectItem key={x} value={x}>{t(x)}</SelectItem>)}</SelectContent>
                  </Select></div>
                {isPartTime && (
                  <div><Label className="text-[11px] uppercase text-slate-500">{t("Taux d'emploi (%)")}</Label>
                ...
                <div className="col-span-2"><Label className="text-[11px] uppercase text-slate-500">{t("Département (cette version)")}</Label>
              ...
              <p className="border-t border-slate-200 px-3 py-2 text-[11px] text-slate-500">{t("Le département/type d'emploi sont propres à cette version de budget. Le changement de département n'affecte que l'imputation (les déductions restent selon la classe de sécurité). Au verrouillage, le département de la fiche employé prendra celui de la version verrouillée.")}</p>
            </div>
            <div className="rounded-xl border border-slate-200">
              ...
              <div className="grid grid-cols-2 gap-3 p-3">
                <div><Label className="text-[11px] uppercase text-slate-500">{t("Salaire de base ($)")}{revueMode && <span className="ml-1 normal-case text-slate-400">{t("— salaire actuel")}</span>}</Label>
                ...
                <div><Label className="text-[11px] uppercase text-slate-500">{t("Augmentation (%)")}</Label>
                ...
                <div><Label className="text-[11px] uppercase text-slate-500">{t("Taux vacances (%)")}</Label>
                ...
                <div className="col-span-2"><Label className="text-[11px] uppercase text-slate-500">{t("Date de changement de salaire (optionnel)")}</Label>
                  ...
                  <p className="mt-1 text-[10px] text-slate-400">{t("Avant cette date, le salaire actuel de base s'applique ; le nouveau salaire s'applique à partir de cette date (proratisé sur l'année).")}</p></div>
                {isCCQ && (
                  <div><Label className="text-[11px] uppercase text-slate-500">{t("Type de prime")}</Label>
            ...
            <div className="rounded-xl border border-slate-200">
              <div className="border-b border-slate-200 bg-slate-50 px-3 py-2"><h3 className="text-xs font-700 uppercase tracking-widest">{isCCQ ? t("Primes & Allocations") : t("Rémunération additionnelle")}</h3></div>
              {isCCQ && (
                <div className="grid grid-cols-3 gap-2 p-3">
                  {[["prime_garde", "Prime de garde"], ["prime_halo", "Prime HALO 5%"], ["alloc_securite", "Alloc. sécurité"]].map(([k, lbl]) => (
                    <label key={k} className="flex items-center justify-between gap-2 rounded-lg border border-slate-200 px-2.5 py-2">
                      <span className="text-[11px] font-500">{t(lbl)}</span><Switch data-testid={`fiche-${k}`} checked={f[k]} onCheckedChange={(v) => set(k, v)} />
                    </label>
                  ))}
                </div>
              )}
              {!isCCQ && (
                <div className="p-3">
                  <div className="grid grid-cols-3 gap-3">
                    <div><Label className="text-[11px] uppercase text-slate-500">{t("RPDB/REER ($)")}</Label><Input data-testid="fiche-reer" type="number" className="mt-1 font-mono-data" value={f.reer} onChange={(e) => set("reer", e.target.value)} /></div>
                    <div><Label className="text-[11px] uppercase text-slate-500">{t("Assu. coll. ($)")}</Label><Input data-testid="fiche-assurance" type="number" className="mt-1 font-mono-data" value={f.assurance} onChange={(e) => set("assurance", e.target.value)} /></div>
                    <div><Label className="text-[11px] uppercase text-slate-500">{t("Boni")}</Label>
                  ...
                  <label className="mt-3 flex w-full items-center justify-between gap-2 rounded-lg border border-slate-200 px-3 py-2 sm:w-1/3">
                    <span className="text-[11px] font-500">{t("Alloc. sécurité")}</span><Switch data-testid="fiche-alloc_securite" checked={f.alloc_securite} onCheckedChange={(v) => set("alloc_securite", v)} />
                  </label>
                  <div className="mt-3 grid grid-cols-2 gap-3">
                    <div><Label className="text-[11px] uppercase text-slate-500">{t("Prime Tedy ($)")}</Label><Input data-testid="fiche-tedy" type="number" className="mt-1 font-mono-data" value={f.tedy} onChange={(e) => set("tedy", e.target.value)} /></div>
                    <div><Label className="text-[11px] uppercase text-slate-500">{t("Prime Telus ($)")}</Label><Input data-testid="fiche-telus" type="number" className="mt-1 font-mono-data" value={f.telus} onChange={(e) => set("telus", e.target.value)} /></div>
                  </div>
                  <p className="mt-2 text-[10px] text-slate-400">{t("Tedy et Telus : montants fixes inclus uniquement dans les bases RRQ / FSS / RQAP / CSST.")}</p>
                </div>
              )}
          ...
          <div className="space-y-4">
            <div className="rounded-xl border border-slate-200">
              <div className="flex items-center justify-between border-b border-slate-200 bg-slate-50 px-3 py-2">
                <h3 className="text-xs font-700 uppercase tracking-widest">{t("Calcul automatique")}</h3>
                {manualAllowed && (
                  <label className="flex items-center gap-2" data-testid="fiche-manual-toggle-label">
                    <span className={`text-[10px] font-700 uppercase ${manualOn ? "text-amber-600" : "text-slate-400"}`}>{t("Saisie manuelle")}</span>
              ...
              <div className="divide-y divide-slate-100">
                ...
                {isCCQ && <CalcRow label={t("Prime de garde")} mkey="garde" value={p.garde} editable={manualOn} manual={manual} setManualField={setManualField} testId="manual-garde" />}
                {isCCQ && <CalcRow label={t("Prime HALO")} mkey="halo" value={p.halo} editable={manualOn} manual={manual} setManualField={setManualField} testId="manual-halo" />}
                ...
                {!isCCQ && <CalcRow label={t("Boni")} mkey="boni" value={p.boni} editable={manualOn} manual={manual} setManualField={setManualField} testId="manual-boni" />}
            ...
            <div className="rounded-xl bg-[#0E1526] px-4 py-3">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-600 uppercase tracking-widest text-white/60">{t("Masse salariale (année pleine)")}</span>
        ...
        {p.monthly && (
          <div className="rounded-xl border border-slate-200" data-testid="fiche-ventilation">
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 bg-slate-50 px-3 py-2">
              <h3 className="text-xs font-700 uppercase tracking-widest">{t("Ventilation mensuelle")} {year}</h3>
              {p.prorated ? (() => {
                ...
                let txt;
                if (late && early) txt = `${t("Actif de")} ${MONTHS[first]} ${t("à")} ${MONTHS[last]} (${p.months_active} ${t("mois")})`;
                else if (early) txt = `${t("Fin d'emploi en cours d'année → actif jusqu'à")} ${MONTHS[last]} (${p.months_active} ${t("mois")})`;
                else txt = `${t("Embauche en cours d'année → pro-rata dès")} ${MONTHS[first]} (${p.months_active} ${t("mois")})`;
                return <span className="text-[11px] text-[#B45309]">{txt}</span>;
              })() : <span className="text-[11px] text-slate-400">{t("Répartie selon les jours ouvrables")}</span>}
            </div>
