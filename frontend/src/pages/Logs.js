...
export default function Logs() {
  ...
  useEffect(() => { api.getLogs().then(setEntries); }, []);
  const fmtDate = (iso) => {
  ...
  const exportCsv = () => {
    ...
    const a = document.createElement("a"); a.href = url; a.download = `logs_${new Date().toISOString().slice(0, 10)}.csv`; a.click(); URL.revokeObjectURL(url);
  };
    ...
    <div className="space-y-4" data-testid="logs-page">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <select value={entityFilter} onChange={(e) => setEntityFilter(e.target.value)} data-testid="logs-filter-entity"
          ...
          <select value={userFilter} onChange={(e) => setUserFilter(e.target.value)} data-testid="logs-filter-user"
            className="h-9 rounded-lg border border-slate-300 bg-white px-3 text-sm text-slate-700 focus:border-[#22C55E] focus:outline-none">
            <option value="">{t("Tous les utilisateurs")}</option>
            {userOptions.map((o) => <option key={o} value={o}>{o}</option>)}
          </select>
          <button onClick={exportCsv} disabled={!filtered.length} data-testid="logs-export-csv"
      ...
      <div className="card overflow-hidden">
        ...
        <ul>
          {filtered.map((e) => {
            ...
            const Icon = st.icon;
            return (
              <li key={e.id} className="flex items-start gap-4 border-b border-slate-100 px-5 py-3.5 last:border-0" data-testid="logs-entry">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {Array.isArray(e.changes) && e.changes.length > 0 && (
                    <ul className="mt-1.5 flex flex-col gap-1" data-testid="logs-changes">
                      {e.changes.map((c, i) => (
