...
function UserForm({ open, onOpenChange, initial, onSubmit }) {
  ...
  const submit = () => {
    const e = {};
    if (!initial && !f.email.trim()) e.email = "Courriel requis";
    if (!f.name.trim()) e.name = "Nom requis";
    if (!initial && f.password.length < 6) e.password = "6 caractères minimum";
    if (initial && f.password && f.password.length < 6) e.password = "6 caractères minimum";
    setErr(e);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent data-testid="user-form-dialog">
        <DialogHeader>
          <DialogTitle>{initial ? `Modifier — ${initial.name}` : "Nouvel utilisateur"}</DialogTitle>
          <DialogDescription className="text-xs">{initial ? "Laissez le mot de passe vide pour le conserver." : "Créez un compte Admin ou Utilisateur."}</DialogDescription>
        </DialogHeader>
        <div className="grid grid-cols-1 gap-3 py-1 sm:grid-cols-2">
          <div className="sm:col-span-2"><Label className="text-[11px] uppercase text-slate-500">Courriel</Label>
        ...
        <DialogFooter>
          ...
          <Button data-testid="user-save-btn" className="bg-[#2563EB] hover:bg-[#2563EB]/90" onClick={submit}>Enregistrer</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
...
export default function Users() {
  ...
  const load = () => api.listUsers().then(setUsers).catch(() => toast.error("Accès refusé"));
  useEffect(() => { load(); }, []);

  const submit = async (data) => {
    try {
      if (dialog.item) { await api.updateUser(dialog.item.id, data); toast.success("Utilisateur mis à jour"); }
      else { await api.createUser(data); toast.success("Utilisateur créé"); }
      setDialog({ open: false, item: null }); load();
    } catch (e) { toast.error(e.response?.data?.detail || "Erreur"); }
  };
  const del = async (u) => { try { await api.deleteUser(u.id); toast.success("Utilisateur supprimé"); load(); } catch (e) { toast.error(e.response?.data?.detail || "Erreur"); } };

  return (
    <div className="space-y-4" data-testid="users-page">
      <div className="flex items-center justify-between">
        <p className="text-sm text-slate-500"><b className="text-slate-800">{users.length}</b> compte(s) — l'admin gère les accès. Les utilisateurs ne peuvent pas modifier un budget verrouillé.</p>
        <Button data-testid="add-user-btn" className="gap-1.5 bg-[#2563EB] hover:bg-[#2563EB]/90" onClick={() => setDialog({ open: true, item: null })}><Plus size={16} /> Ajouter</Button>
      </div>
      <div className="card overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-200 text-[11px] uppercase tracking-wider text-slate-400">
              <th className="px-4 py-3 text-left font-600">Nom</th><th className="px-4 py-3 text-left font-600">Courriel</th>
              <th className="px-4 py-3 text-left font-600">Rôle</th><th className="px-4 py-3 text-right font-600">Actions</th>
            </tr>
          </thead>
          <tbody>
            {users.map((u) => (
              <tr key={u.id} className="border-b border-slate-100 hover:bg-slate-50" data-testid={`user-row-${u.email}`}>
                ...
                <td className="px-4 py-2.5">
                  <div className="flex justify-end gap-1">
                    <button data-testid={`edit-user-${u.email}`} onClick={() => setDialog({ open: true, item: u })} className="p-1.5 text-slate-400 hover:text-[#2563EB]"><Pencil size={15} /></button>
            ...
            {users.length === 0 && <tr><td colSpan={4} className="px-4 py-10 text-center text-sm text-slate-500">Aucun utilisateur.</td></tr>}
          </tbody>
        </table>
      </div>
