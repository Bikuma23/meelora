...
- [x] **Sélecteur d'affichage des colonnes P&L** (« Mois / Cumulatif / Les deux », `acct-blockview-select`) : filtre les blocs de colonnes pour une lecture confortable (le tableau à 16 colonnes peut être réduit à 8). Uniquement pour le P&L (présence de `col_groups`). Vérifié visuellement (mode Cumulatif = 8 colonnes du bloc cumulatif).
