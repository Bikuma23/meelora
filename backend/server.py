...
# ---------------------------------------------------------------------------
# Surcouche IA (Phase 3) — additive, réversible, ne modifie aucune donnée financière
# ---------------------------------------------------------------------------
import ai_service
...
@api.get("/acct/ai/status")
async def acct_ai_status(user: dict = Depends(get_current_user)):
    return ai_service.ai_status(await _ai_config())

@api.get("/acct/ai/config")
async def acct_ai_get_config(user: dict = Depends(require_admin)):
    return _ai_config_public(await _ai_config())

@api.put("/acct/ai/config")
async def acct_ai_put_config(body: AIConfigBody, user: dict = Depends(require_admin)):
    provided = body.dict(exclude_unset=True)
    sets = {}
...
@api.delete("/acct/ai/config/key")
async def acct_ai_clear_keys(user: dict = Depends(require_admin)):
...
@api.post("/acct/ai/variance")
async def acct_ai_variance(year: int, month: int, user: dict = Depends(get_current_user)):
...
@api.post("/acct/ai/anomalies")
async def acct_ai_anomalies(year: int, month: int, user: dict = Depends(get_current_user)):
    cfg = await _ai_config()
...
@api.post("/acct/ai/chat")
async def acct_ai_chat(body: AIChatBody, user: dict = Depends(get_current_user)):
    cfg = await _ai_config()
    st = ai_service.ai_status(cfg)
    if not st["configured"]:
...
@api.get("/acct/ai/chat/history")
async def acct_ai_chat_history(session_id: str, user: dict = Depends(get_current_user)):
...
@api.post("/acct/ai/suggest-mapping")
async def acct_ai_suggest_mapping(body: AISuggestBody, user: dict = Depends(get_current_user)):
    cfg = await _ai_config()
    st = ai_service.ai_status(cfg)
    if not st["configured"]:
        return {"available": False, "reason": "Fonctionnalité IA non configurée."}
